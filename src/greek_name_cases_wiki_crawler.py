...
def crawl_names(session, dict_500, domain_URL, url, c, listing_category):

    while url != None:
        print('Crawling ', urllib.parse.urljoin(domain_URL, url))
        response = get_with_retries(session, urllib.parse.urljoin(domain_URL, url))
        ...
        for entry in entries_list:
            ...
            # be polite to the volunteer-run site
            time.sleep(0.5)
            response = get_with_retries(session, urllib.parse.urljoin(domain_URL,name_url))
...
for dirpath, url_category in dirs_urls_dict.items():
    dir_name = os.path.basename(dirpath)
    page_url = url_category[0]
    listing_category = url_category[1]
    c=0
    dict_500 = {}
    crawl_names(session, dict_500, domain_URL, page_url, c, listing_category)
