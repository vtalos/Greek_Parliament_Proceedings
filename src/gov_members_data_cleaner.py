...
import numpy as np
import datetime
from common import text_formatting
...
def specific_corrections(df):
    ...
    d3 = {'στεφανος μανου': 'στεφανος μανος', 'αθανασιος νακου':'αθανασιος νακος',
         ...
         'χρηστος ροκοφυλλου' : 'χρηστος ροκοφυλλος', 'χρηστος-γεωργιος σκερτσου' : 'χρηστος-γεωργιος σκερτσος',
          ...
          'ζωη ραπτης': 'ζωη ραπτη', r'\bγιατρωμανωλακης\b': 'γιατρομανωλακης',
          'μακαριο λαζαριδης': 'μακαριος λαζαριδης'
          }
...
def first_name_formatting(df):
    ...
    d = {r'\bελπιδα\b': 'ελπιδας', r'\bαθανασια\b': 'αθανασιας', r'\bφωτεινη\b':'φωτεινης',
         ...
         r'\bζωη\b': 'ζωης', r'\bευαγγελο\b': 'ευαγγελου', r'\bβασιλειος\b': 'βασιλειου'
...
def name_formatting_dataframe(df):
    ...
    df['member_name'] = df['member_name'].str.replace(r'\x96', '-', regex=True)
    df['member_name'] = df['member_name'].str.replace(r'\s+\-\s+', '-', regex=True)

    # Add whitespace before parenthesis. (\S) matches any non-whitespace character.
    df['member_name'] = df['member_name'].str.replace(r"(\S)\(", r'\1 (', regex=True)
    # remove father's name in parenthesis
    regex_fatherName = re.compile(r'(^.*)(\s\((του)\s.*?\)$)')
    df['member_name'] = df['member_name'].str.replace(regex_fatherName, r'\1', regex=True)

    # rearrange order of string parts with capture groups. move parentheses to end of name
    regex_parenthesis = re.compile(r'(^.*)(\s\(.*?\))(.*$)')
    df['member_name'] = df['member_name'].str.replace(
        regex_parenthesis, r'\1\3\2', regex=True)

    df['member_name'] = df['member_name'].str.replace(
        r'( γ\. )|( α\. )|( οθ\. )|( συζ\.δημητριου )|( σπ\. )|( σπ\.)', ' ', regex=True)
    df['member_name'] = df['member_name'].str.replace(
        r'\(πρωθυπουργου \)', '', regex=True)
    df['member_name'] = df['member_name'].str.replace(
        r'\(πρωθυπουργου \)', '', regex=True)
    df['member_name'] = df['member_name'].str.replace(
        r'αποστολ\.|αποστ\.|αποστολουυ', 'αποστολου', regex=True)
    ...
    df['member_name'] = df['member_name'].str.replace('ανασταστιου',
                                                                'αναστασιου')
    df['member_name'] = df['member_name'].str.replace(
        r'νικολ\.φωτιου χατζημιχαλη', 'νικολαου-φωτιου χατζημιχαλη', regex=True)
    df['member_name'] = df['member_name'].str.replace(
        r'(μαρια-κολλια τσαρουχα)|(μαριας κολλια τσαρουχα)',
        'μαριας κολλια-τσαρουχα', regex=True)
    df['member_name'] = df['member_name'].str.replace(
        ...
        'αποστολου-αθανασιου τσοχατζοπουλου', regex=True)
...
df['nickname'] = df['nickname'].str.replace(r'[\(\)]', '', regex=True)
df['gender'] = np.nan
...
df['member_role'] = df['member_role'].str.replace(r'^και\s', '', regex=True)
...
new_rows = [
    ...
    ['2012-06-20', 'τελος_κυβερνησης', 'γεωργιου ζανια', 'υπουργος οικονομικων',
     '2012-05-16', '2012-06-20', 'πικραμμενου παναγιωτη οθ. (υπηρεσιακη)',
     'γεωργιος', 'ζανιας', np.nan, 'male', 'γεωργιος ζανιας'],
    ['2012-06-20', 'αρχη_κυβερνησης', 'γεωργιου ζανια', 'υπουργος οικονομικων',
     '2012-06-20','2015-01-26','σαμαρα κ. αντωνιου',
     'γεωργιος', 'ζανιας', np.nan, 'male',  'γεωργιος ζανιας']
]
...
for name, subdf in ministers_without_portfolio_1990.groupby(['cleaned_fullname']):
    if subdf.shape[0] == 1 and subdf['event'].iloc[0] == 'διορισμος':
        copy = subdf.copy()
        new_subdf = pd.concat([copy, copy], ignore_index=True)
        # we can use .at because index is reset
        new_subdf.at[0, 'event'] = 'παραιτηση'
        new_subdf.at[0, 'date'] = event_date - datetime.timedelta(days=1)
        new_subdf.at[1, 'event'] = 'διορισμος'
        new_subdf.at[1, 'date'] = event_date
        new_subdf.at[1, 'member_role'] = 'υπουργος επικρατειας'
...
df = correct_specific_events(df, 'κωνσταντινος κουκοδημος', 'υφυπουργος παιδειας και θρησκευματων',
                             'διορισμος', 'παραιτηση', '2014-09-02', '2012-06-20')

# the last event of the caretaker Sarmas government is wrongly split on the website
df = correct_specific_events(df, 'ιωαννης σαρμας', 'πρωθυπουργος',
                             'παρα', 'παραιτηση', '2023-06-26', '2023-05-25')
...
df = correct_specific_roles(df, 'κωνσταντινος μουσουρουλης', 'υπουργος ναυτιλιας και αιγαιου',
                       'υπουργος ναυτιλιας','2013-06-25', '2012-06-20')

# the finance ministry was renamed to εθνικης οικονομιας και οικονομικων in 06/2024,
# during the terms of office of the following members
df = correct_specific_roles(df, 'κωνσταντινος χατζηδακης', 'υπουργος εθνικης οικονομιας και οικονομικων',
                       'υπουργος οικονομικων','2025-03-14', '2023-06-26')

df = correct_specific_roles(df, 'θεοχαρης θεοχαρης', 'υφυπουργος εθνικης οικονομιας και οικονομικων',
                       'υφυπουργος οικονομικων','2024-06-14', '2023-06-26')
...
for name, subdf in last_gov_events.groupby(['cleaned_fullname', 'member_role']):
    if subdf.shape[0] == 1 and subdf['event'].iloc[0] == 'διορισμος':
        ...
        copy.iat[0, copy.columns.get_loc('event')] = 'last_script_run'
        copy.iat[0, copy.columns.get_loc('date')] = copy['gov_date_to'].iat[0]
...
last_gov_date_to = df.gov_date_to.max()

# match assignment and resignation dates for each role
# the date column is kept as datetime here, so that sorting is chronological
# and not alphabetical, and is formatted as 27/01/2001 on output
for name, subdf in df.groupby(['cleaned_fullname','gov_date_from']):
    ...
    for role in set(subdf.member_role.to_list()):
        ...
        assignment_dates = list(role_subdf.loc[(role_subdf.event.isin(start_events)),'date'])
        resignation_dates = list(role_subdf.loc[(role_subdf.event.isin(end_events)),'date'])

        # a member can be assigned the same role more than once within a government
        for i, assignment_date in enumerate(assignment_dates):

            if i < len(resignation_dates):
                resignation_date = resignation_dates[i]
            elif subdf.iloc[0].gov_date_to == last_gov_date_to:
                resignation_date = subdf.iloc[0].gov_date_to #still in office
            else:
                continue

            final_list.append([subdf.iloc[0].cleaned_fullname, role,
                               assignment_date.strftime('%d/%m/%Y'),
                               resignation_date.strftime('%d/%m/%Y'),
                               subdf.iloc[0].gender])
